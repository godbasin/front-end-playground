...
这里我们需要排除`@angular/compiler`以及`rxjs`，可能还有其他一些依赖，不然会有 webpack 的`warning`。详细也可以查看类似的[issue-Warnings displayed by webpack when using source-map-loader](https://github.com/angular-redux/store/issues/64)。
