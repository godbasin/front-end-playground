...
首先，我们需要通过一个位置坐标(x, y)来获取该坐标下的刚体，若无则返回 null。
...
function handleMousedown(e) {
  ...
  var selectBody = GetBodyAtPosition(
    newMouse.x - canvas.offsetLeft,
    newMouse.y - canvas.offsetTop
  ); // 选择刚体
  if (selectBody) {
    // 处理选中的刚体
  } else {
...
[此处查看项目代码（仅包含 src 部分）](https://github.com/godbasin/box2djs-tutorial/tree/master/6-practice/6-4-mouse-operate-body/6-4-mouse-operate-body)  
